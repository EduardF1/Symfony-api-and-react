...
use App13\Format13\JSON;
use App13\Format13\XML;
...
class Kernel {
    ...
    private function bootContainer(Container $container)
    {
        ...
        $container->loadServices('App13\\Service');

        // deprecated but with no alternative now, will be replaced in 2.0 (Doctrine v.)
    ...
    public function handleRequest()
    {
        ...
        if (isset($this->routes[$uri])) {
            ...
            $response = $this->container->getService($route['service'])
                // call the service method from the route object
                ->{$route['method']}();
            echo $response;
            die;
        }
